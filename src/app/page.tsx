...
function generateFallbackResult({
  ...
  const scoredArchetypes = [
    { name: "Проводник", score: tagCount(["знак", "поиск", "выбор", "глубина"]) },
    { name: "Наблюдатель", score: tagCount(["наблюдение", "ум", "дистанция", "контроль"]) },
    { name: "Творец", score: tagCount(["рост", "радость", "проявленность", "игра"]) },
    ...
    { name: "Игрок", score: tagCount(["юмор", "абсурд", "лёгкость", "социальность"]) },
    { name: "Собирающий себя", score: tagCount(["границы", "тело", "тишина", "уязвимость"]) },
    { name: "Человек перехода", score: tagCount(["выбор", "неясность", "границы", "рост"]) },
  ];
  const archetype =
  ...
  const artifact = uniqueTags.includes("тишина")
  ...
  const task = answerText.match(/страш|бою|страх/)
...
Главный зов
Первая выбранная картинка — ${firstChoice?.name ?? "образ"} — показывает то, что сейчас сильнее всего притягивает ваше внимание. В её тегах звучат: ${firstChoice?.tags.join(", ") ?? "внимание, движение, поиск"}. Это не обязательно прямая цель. Скорее, это направление, от которого внутри становится чуть тише и собраннее, потому что оно напоминает о важной части вас.

В вашем выборе считывается движение ${genderTone}. Ответы показывают, что сейчас важно не просто получить готовый совет, а увидеть, что именно просит места. Главный зов похож на внутренний сигнал: «посмотри сюда внимательнее, здесь есть нечто живое». Даже если внешне всё ещё не оформлено, внутри уже есть притяжение к более точному способу жить и выбирать.

Этот образ говорит не о том, кем вы обязаны стать, а о том, что сейчас хочет проявиться. В нём есть сила не давить на себя, а распознать главный импульс и дать ему форму.

Внутренняя потребность
Вторая выбранная картинка — ${secondChoice?.name ?? "образ"} — раскрывает слой потребности. Её теги: ${secondChoice?.tags.join(", ") ?? "тепло, ясность, опора"}. Здесь речь не о внешнем результате, а о том, какая часть вас просит внимания: та, которой нужно тепло, ясность, признание, движение или право на паузу.

Похоже, внутри есть запрос на более бережное обращение с собой. Не на слабость и не на остановку, а на честную настройку: где вы действуете из живого желания, а где продолжаете тянуть то, что уже не питает. Эта потребность может быть тихой, но она важна, потому что без неё главный зов превращается в напряжение.

Если прислушаться к этому слою, можно заметить: вам сейчас важно не ускоряться любой ценой, а вернуть себе контакт с тем, что действительно ваше. В этом месте появляется опора.

Точка перехода
Третья выбранная картинка — ${thirdChoice?.name ?? "образ"} — обозначает точку перехода. Её теги: ${thirdChoice?.tags.join(", ") ?? "граница, шаг, выбор"}. Это место, где может быть сопротивление, тень или страх сделать следующий шаг. Но именно здесь чаще всего скрыта дверь.

Точка перехода не требует резкого рывка. Она показывает, где старая стратегия уже стала тесной. Возможно, вы привыкли ждать полной ясности, собирать больше подтверждений или держать внутреннее движение в тени. Но выбранный образ говорит: ближайший шаг не обязан быть большим. Он должен быть честным.

Здесь важно увидеть напряжение не как проблему, а как границу роста. То, что сопротивляется, часто не мешает вам, а защищает от слишком резкого изменения. С ним можно договориться.

Ваш предварительный образ
Предварительный образ сейчас ближе к архетипу «${archetype}». Он складывается из повторяющихся оттенков: ${uniqueTags.slice(0, 10).join(", ")}. В этом образе есть ваша текущая сила: не абстрактный потенциал, а конкретный способ собирать себя в моменте.

Этот архетип показывает, что сейчас вы не стоите на месте. Вы находитесь в процессе внутренней сборки: что-то уже стало прежним, а новое ещё требует языка, формы и первого действия. Ваша сила — в способности замечать нюансы и не предавать внутреннюю правду ради быстрых ответов.
...
Сейчас этот механизм можно не ломать, а смягчать. Не требовать от себя мгновенной смелости, а создавать условия, в которых движение становится безопаснее. Иногда рост начинается не с сильного решения, а с разрешения сделать маленький шаг без идеальной уверенности.
...
После этого не оценивайте себя строго. Просто отметьте, что изменилось в теле, настроении и ощущении направления. Метаграф здесь не просит доказательств. Он предлагает увидеть движение.
